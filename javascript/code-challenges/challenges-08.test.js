...
const sortByChildren = (charArray) => {
  charArray.sort(charArray.children.length);
  if (charArray.children.length === charArray.children.length) {
    charArray.sort(charArray.children.house);
  }
  return charArray;
};
...
const isCapitalized = (str) => {
  let matches = /[A-Z][a-zA-Z]*/g;
  // let matchArr = str.match(' ');
  // let results = matchArr.filter(word => matches.test(word));
  // let finalResults = results.map(word => word)
  let results = str.match(matches);
  // if str.includes
  return results || [];

  // str.map((matches.test(str)) => {
  //   let question = arr.map(newString => `${newString}?`);
  //   return question;  
  // })
};
