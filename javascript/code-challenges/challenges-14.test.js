...
------------------------------------------------------------------------------------------------ */
// worked with Phil, Alex, Charlie
const screenForNames = (arr) => {
  let names = [];
  let regex = /^(M|D)(r|s)(. |s. )[a-zA-Z]/gm;
  arr.map(name => {
    if (regex.test(name)) {
      names.push(name);
    }
...
const toTitleCase = (arr) => {
  return arr.map(string => { return string.charAt(0).toUpperCase() + string.slice(1);
  });
};
...
let biggerThanLuke = (arr) => {
  return arr.filter((character) => Number(character.mass) > 77).map(person => person.name).join(' - ');
};
