...
// Ryan helped with the double reduce
const count = (target, input) => {
  return input.reduce((acc, currentArr) => {
    let rowCounts = currentArr.reduce((innerAcc, innerCurrentVal) => {
      if (innerCurrentVal === target) {
        return innerAcc + 1;
      }
      return innerAcc;
    }, 0);
    return acc + rowCounts;
  }, 0);
};
...
// Then I used what I learned on #2 to rock #3.
const totalSum = (input) => {
  return input.reduce((acc, currentArr) => {
    let rowCounts = currentArr.reduce((innerAcc, innerCurrentVal) => {
      innerAcc = innerAcc + innerCurrentVal;
      return innerAcc;
    }, 0);
    return acc + rowCounts;
  }, 0);
